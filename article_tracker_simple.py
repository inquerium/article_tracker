...
class ArticleTracker:
    ...
    def check_duplicates(self, title, content, keywords):
        ...
        # 1. Check for duplicate/similar title (exact match or contains)
        cur = self.conn.cursor()
        cur.execute("""
        SELECT id, title
        FROM articles
        ...
        # Only look for existing articles (not the one being currently checked)
        cur.execute("""
        ...
        if has_exact_match:
            ...
            # Get the duplicate article details
            cur.execute("""
        ...
        # Then check for similar content using tsquery
        content_preview = " ".join(content.split()[:200])  # First 200 words
        # Extract important words for comparison
        words = [w.lower() for w in word_tokenize(content_preview) 
                if w.isalnum() and w.lower() not in stop_words and len(w) > 3]
        
        # Take top words for comparison
        top_words = words[:10]
        if top_words:
            # Create tsquery string with OR between words
            ...
            # Only consider as duplicates if similarity is at least 0.25 (25%)
            similar_content = []
            for row in cur.fetchall():
                similarity_score = float(row[2])/10.0
                if similarity_score >= 0.25:  # 25% threshold
            ...
            if similar_content:
                ...
                for row in similar_content:
                    result["content_duplicates"].append({
                        "id": row[0],
                        "title": row[1],
                        "similarity": float(row[2])/10.0  # Convert to similarity scale
                    })
    ...
    def add_article(self, title, content, author, department=None, 
        ...
        try:
            cur = self.conn.cursor()
            # Debug - check inputs
            logger.info(f"Inserting article: '{title}', author: {author}, keywords: {keywords}")
    ...
    def get_recent_articles(self, days=30, department=None, limit=100):
        ...
        if department and department != "All":
            ...
            LIMIT %s
            """
            df = pd.read_sql(query, self.engine, params=(days, department, limit))
        else:
            query = """
            SELECT id, title, author, department, publish_date, keywords
            FROM articles
